...
export async function fetchDeclaredIncomes(): Promise<DeclaredIncome[]> {
  ...
  return data.map((r) => ({ id: r.id as string, name: r.name as string, amountCents: r.amount_cents as number, active: Boolean(r.active) }))
}
...
export async function createDeclaredIncome(name: string, amountCents: number): Promise<string | null> {
  ...
  const { error } = await supabase.from('declared_incomes').insert({ user_id: user.id, name: name.trim(), amount_cents: amountCents })
  if (error) {
